---
name: aigc
description: AI 生成图片和视频。当用户说"画一张"、"生成图片"、"做个视频"、"帮我画"、"生成一张图"、"文生图"、"文生视频"、"图生视频"、"视频生视频"时，必须先加载此 skill 了解用法
metadata: {"nanobot":{"emoji":"🎨"}}
---

# 图像和视频生成

**直接调用 `aigc` 工具**生成图片和视频（阿里云通义万相）。

## ⚠️ 重要规则

- **必须直接调用 `aigc` 工具**，不要使用 task、shell 或其他工具
...
**用户**: 帮我画一只可爱的小猫
**直接调用 aigc 工具**: `{"action": "generate_image", "prompt": "一只可爱的猫咪，卡通风格"}`

**用户**: 把这张图片变成动起来的视频
**直接调用 aigc 工具**: `{"action": "generate_video_from_image", "prompt": "画面开始动起来", "image_path": "..."}`
