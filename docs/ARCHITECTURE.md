...
| Docker | ⏳ | 容器化部署 |
| ClawHub 技能库 | ⏳ | 技能搜索安装 |
| LLM 驱动记忆管理 | ⏳ | 参考 Mem0 的智能记忆决策 |

#### LLM 驱动记忆管理（参考 Mem0）

当前 LingGuard 使用简单的规则匹配（字符串相似度 ≥0.95）进行记忆去重。Mem0 的创新方案是使用 LLM 智能决策：

**核心机制：**
1. **事实提取**：LLM 从消息中提取独立的结构化事实
2. **操作决策**：LLM 对每条事实决定执行哪种操作
   - `ADD`：新增记忆（完全新的信息）
   - `UPDATE`：更新已有记忆（修改/补充）
   - `DELETE`：删除过时记忆（矛盾信息）
   - `NONE`：无需存储（寒暄、无意义内容）
3. **程序性记忆**：LLM 总结用户习惯和行为模式

**实现思路：**
```
用户消息 → LLM 事实提取 → 搜索相似记忆 → LLM 决策(ADD/UPDATE/DELETE/NONE) → 执行操作
```

**与当前方案对比：**

| 特性 | Mem0 (LLM 驱动) | LingGuard (当前) |
|------|-----------------|------------------|
| 记忆决策 | LLM 动态判断 | 规则匹配（相似度阈值） |
| 事实提取 | LLM 结构化提取 | 直接存储原始内容 |
| 记忆更新 | 智能合并、版本追踪 | 新增条目（不更新） |
| 记忆删除 | LLM 识别过时信息 | 无自动删除 |
| 成本 | 高（每次操作调用 LLM） | 低（仅 embedding） |

**实现文件：**
- `pkg/memory/llm_manager.go` - LLM 记忆管理器
- `pkg/memory/prompts.go` - 事实提取/决策 Prompt
- 扩展 `HybridStore` 支持 UPDATE/DELETE 操作
